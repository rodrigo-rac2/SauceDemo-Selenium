...
# -c selenium_config.cfg   - This defines the pytest configuration file.
...
#########################################################################################
if [ "$1" != "" ]; then
  python -m pytest -c selenium_config.cfg --html=./log/report.html --self-contained-html -n 4 -m $1
else
  python -m pytest -c selenium_config.cfg --html=./log/report.html --self-contained-html -n 4
fi
